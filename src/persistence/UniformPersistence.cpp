...
namespace {
    ...
    struct {
        ...
        const char* referenceType         = "referenceType";
    ...
    std::optional<InspectorReferenceType> parseReferenceType(const std::string& s) {
        for (int i = static_cast<int>(InspectorReferenceType::Uniform); i <= static_cast<int>(InspectorReferenceType::SceneVariable); ++i) {
            auto t = static_cast<InspectorReferenceType>(i);
            if (to_string(t) == s) {
                return t;
            }
        }
            ...
            overloaded{
                ...
                [](const InspectorReference& ref) {
                    return json{
                        ...
                        { referenceLabels.uniformSelection, ref.valueSelection },
                        ...
                        { referenceLabels.referencedUniformName, ref.referencedValueName },
                        { referenceLabels.returnType, to_string(ref.returnType) },
                        { referenceLabels.referenceType, to_string(ref.referenceType) },
    ...
    bool jsonToUniformValue(const json& j, UniformValue& out) {
        ...
        try {
            ...
            if (kind == uniformLabels.valueKindReference) {
                ...
                ref.valueSelection     = j.value(referenceLabels.uniformSelection, 0);
                ...
                if (auto rt = parseReferenceType(
                    j.value(referenceLabels.referenceType, std::string(to_string(InspectorReferenceType::Uniform))))) {
                    ref.referenceType = *rt;
                }
                else {
                    ref.initialized   = false; 
                    ref.referenceType = InspectorReferenceType::Uniform;
                }
                out               = ref;
                return true;
            }
