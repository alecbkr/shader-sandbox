...
void Application::initializeUI(AppContext& ctx) {
    ...
    ctx.settingsModal.initialize(&ctx.logger, &ctx.inputs, &ctx.keybinds, &ctx.platform, &ctx.settings, &ctx.project, &ctx.events);
...
bool Application::initialize(AppContext& ctx) {
    ...
    if (!ctx.shader_registry.initialize(&ctx.logger, &ctx.events, &ctx.project)) {
