...
void ObjCache::createObj(const std::string& name, std::vector<float> verts, std::vector<int> indices, 
    ...
    if (getObject(name) != nullptr) {
        Logger::addLog(LogLevel::WARNING, "OBJECT CACHE", "Object creation failed. Name already in use:", name.c_str());
        return;
    }
...
void ObjCache::translateObj(const std::string name, glm::vec3 pos) {
    ...
    if (obj == nullptr) {
        Logger::addLog(LogLevel::WARNING, "OBJECT CACHE", "Object not found:", name.c_str());
        return;
    }
    obj->translate(pos);
}
...
void ObjCache::setName(const std::string name, const std::string newName) {
    ...
    if (getObject(newName) != nullptr) {
        Logger::addLog(LogLevel::WARNING, "OBJECT CACHE", "Name already in use:", name.c_str());
        return;
    }
